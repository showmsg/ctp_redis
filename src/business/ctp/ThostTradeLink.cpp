...
QryInstrumentMap    m_qryInstrumentMap; //< 同一组只允许一路查询合约
...
{
    char sTemp[1024];	
    sprintf(sTemp,"交易席位[%s]-[%s]连接成功", _brokerID.c_str(), _userID.c_str());
    LOG_INFO("%s \n",sTemp);
    Login();
...
{
    char sTemp[1024];	
    sprintf(sTemp,"交易席位[%s]-[%s]断开,Reason=[%d]",  _brokerID.c_str(), _userID.c_str(), nReason);		
...
LOOP_USERLOGIN:
            ...
            LOG_TRACE("#rsplogin# %s", jsonStr.c_str());
        }
...
void CThostTradeLink::OnRspSettlementInfoConfirm(CThostFtdcSettlementInfoConfirmField *pSettlementInfoConfirm, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
    ...
    {		
        ///请求查询合约
		if(m_qryInstrumentMap[_marketgroup] == 0)
		{
			m_qryInstrumentMap[_marketgroup] = 1;
			ReqContract();			
		}
        else
		{
...
{	
    LOG_TRACE("OnRspOrderAction:%s", pRspInfo->ErrorMsg);
...
{
    LOG_TRACE("OnRtnOrder OrderLocalID:%s , OrderSysID:%s", pOrder->OrderLocalID, pOrder->OrderRef);
...
{
    LOG_TRACE("OnRtnTrade:  OrderLocalID:%s, OrderSysID:%s", pTrade->OrderLocalID, pTrade->OrderRef);
...
void CThostTradeLink::OnRspQryInstrument(CThostFtdcInstrumentField *pInstrument, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
    if(NULL != pInstrument && NULL != _xredis)
    {
        ...
        RedisDBIdx dbi(_xredis);
        LOG_TRACE("instrument:%s, %s", linstrument.c_str(), inst.c_str());
...
LOOP_INSTRUMENTS_HSET:
        ...
        {	
            ...
            LOG_TRACE("market list:%s %s", lmrklist.c_str(), m_instruments.c_str());
